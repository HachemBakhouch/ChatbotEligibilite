...
def process_audio():
    ...
    try:
        ...
        # NOUVEAU CODE: Vérifier l'état de la conversation et traiter le refus de consentement
        current_conversation = conversation_manager.get_conversation(conversation_id)
        if current_conversation:
            current_state = current_conversation.get("current_state", "")

            # Si nous sommes à l'état de consentement et que le texte contient un refus
            if current_state == "consent" and text.lower():
                negative_words = [
                    "non",
                    "pas",
                    "refuse",
                ...
                if any(word in text.lower() for word in negative_words):
                    print("Refus de consentement détecté dans l'entrée audio")

                    # Mettre à jour l'état de la conversation
                    current_conversation["current_state"] = "end"

                    # Ajouter le message de refus à l'historique
                    refusal_message = "Je comprends. Sans ces informations, je ne peux pas déterminer votre éligibilité. N'hésitez pas à revenir si vous changez d'avis."
                    ...
                    # Retourner directement la réponse de refus
                    return (
                        jsonify(
                            {
            ...
            # Si nous sommes déjà dans l'état "end" (refus précédent) et que l'utilisateur change d'avis
                ...
                if any(word in text.lower() for word in positive_words):
                    print("Changement d'avis détecté dans l'entrée audio")

                    # Mettre à jour l'état de la conversation pour revenir à l'âge
                    current_conversation["current_state"] = "age_verification"

                    # Préparer le message pour l'âge
                    ...
                    # Retourner directement la réponse pour l'âge
                    return (
                        jsonify(
                            {
                                "message": age_message,
        ...
        # Traitement normal du message transcrit si aucune condition spéciale n'est rencontrée
        response = conversation_manager.process_message(conversation_id, text)
